...
def launch_docker(name, config, config_path, token, terminator):
    ...
    d2slam_source = config.get("d2slam_source", "")
    ...
    bag_path_raw = config["dataset"][name]["bag"]
    ...
    if "entry_point" in config:
        ...
        if "entry_point_script" in config:
            ...
            print("No entry_point or entry_point_script, exiting...")
            exit(-1)
    ...
    if bag_path_raw == "":
        ...
        bag_path = pathlib.Path(bag_path_raw)
        if bag_path.is_absolute():
            bag_mount = f"-v {bag_path.parent}:/root/bags/{name}_bag"
            bag_arg = f"/root/bags/{name}_bag/{bag_path.name}"
        else:
            bag_mount = f"-v {dataset_path}:/root/bags/"
            bag_arg = f"/root/bags/{bag_path_raw}"

    extra_mounts = []

    # Do not mount the whole workspace unless it really has devel/setup.bash.
    # Mounting the whole workspace will hide the image's built /root/swarm_ws.
    if workspace != "":
        extra_mounts.append(make_mount(workspace, "/root/swarm_ws/"))

    # Your usual Docker style: only override D2SLAM source code.
    # This preserves /root/swarm_ws/devel/setup.bash from the image.
    if d2slam_source != "":
        extra_mounts.append(make_mount(d2slam_source, "/root/swarm_ws/src/D2SLAM"))

    if bag_mount != "":
        extra_mounts.append(bag_mount)
    ...
    if bag_arg == "":
        ...
        docker_args = f"{bag_arg} {token}"

    docker_tty = "-it" if terminator else ""

    cmd = f"""docker run --name {container_name} --gpus all --rm {docker_tty} --net=host \
{extra_mounts_str} \
...
--privileged \
{image_name} /root/docker_entrypoint.sh {docker_args}"""

    print(cmd)

    if terminator:
        p_docker = subprocess.Popen(
            f'terminator -T {container_name} -x "{cmd}"',
            shell=True,
...
def run_swarm_docker_evaluation(config, enable_zsh, config_path, token, terminator):
    ...
    if enable_zsh:
        ...
        for name in config["dataset"]:
            ...
            pids_zsh[name] = subprocess.Popen(
                cmd,
                shell=True,
                stderr=subprocess.STDOUT,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--bash",
        action="store_true",
        help="Open additional bash.",
    )
    parser.add_argument(
        "-t",
        "--terminator",
        action="store_true",
        help="Open docker in terminator.",
    )
    parser.add_argument(
        "-n",
        "--nointeraction",
    ...
    pids, _ = run_swarm_docker_evaluation(
        config,
        args.bash,
        args.config_path,
        token,
    ...
    try:
        ...
        sync_ctrl = SyncCtrl(
            ...
            token=token,
            start_delay=1.0,
            drone_num=len(config["dataset"]),
            enable_interaction=not args.nointeraction,
        )
        sync_ctrl.work()
